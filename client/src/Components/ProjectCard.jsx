...
const ProjectCard = (drive) => {
  ...
  if (left <= 0) return;
  return (
    <div className="flex flex-col justify-center items-center bg-gray-200 rounded-md shadow-2xl cursor-pointer">
      ...
      <div className="w-4/5 flex ml-15 flex-col mb-4">
        <h2 className=" text-[#FFFFFF] font-bold text-xl ">
          {drive.props.title}
        </h2>
      ...
      <div className="w-4/5 flex ml-15 flex-row justify-between">
        <div className="flex flex-col">
          ...
          <span className=" text-[#D0D0D0]">{`Raised of ${web3.utils
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[#D0D0D0]">Days Left</span>
        </div>
      </div>
      <div className="text-xs m-2 text-[#D0D0D0]">{` by ${drive.props.owner}`}</div>
    </div>
  );
};
